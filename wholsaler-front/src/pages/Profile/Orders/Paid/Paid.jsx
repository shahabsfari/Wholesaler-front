...
export default function Paid() {
    ...
    const [currentPage, setCurrentPage] = useState(() => {
        const savedPage = sessionStorage.getItem("selectorInvoiceDeliverdInPAdminPage");
    ...
    const handlePageClick = (event) => {
        ...
        sessionStorage.setItem("selectorInvoiceDeliverdInPAdminPage", selectedPage);
    };
    ...
    const deliverd = invoices.filter((invoice) => invoice.status === 'PAID_DELIVERED');
    ...
    useEffect(() => {
        const fetchAndSetOrders = async () => {
            if (customerData !== null && isLoggedIn) {
                setLoading(true)
                try {
                    const orders = await fetchOrders(itemsPerPage, currentPage, 2);
                    // console.log(orders)
                    ...
                    <div key={index} onClick={() => handleClick(order)} className='cursor-pointer border-[1px] flex flex-col gap-1 border-blue-400/70 rounded-lg p-2 px-4'>
                        ...
                        <div className='hidden w-full sm:flex justify-between items-center' >
                            <span className='flex items-center gap-1 '>
                                <FaBoxOpen className='text-green-600' size={26} />
                                دریافت شده
                            </span>
