...
export default function EditPoster() {
    ...
    const deletePoster = async (posterId) => {
        try {
            ...
            const response = await fetch(`/api/poster/${posterId}`, {
                method: 'DELETE',
            });
            ...
            if (data.status === 'SUCCESS' && !data.hasError) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'خطا!',
                    text: data.message || 'حذف پوستر ناموفق بود.',
                });
            }
        } catch (error) {
            // نمایش پیام خطا
    ...
    const updatePoster = async (posterId) => {
        // تنظیم مقدار enable
        const updatedState = {
            ...state,
                    ...
                    try {
                        ...
                        if (!data.hasError) {
                            Swal.fire({
                                icon: 'success',
                                title: 'موفقیت آمیز!',
                                text: 'پوستر با موفقیت بروزرسانی شد. ',
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'خطا!',
                                text: data.message || 'بروزرسانی پوستر ناموفق بود.',
                            });
                        }
                    } catch (error) {
                        Swal.fire({
    ...
    return (
        <div className='text-white col-span-12 flex flex-col py-2'>
            ...
            <div className='w-full grid grid-cols-12 gap-3 '>
                ...
                <button onClick={() => deletePoster(receivedData.id)} className=' col-span-6 sm:col-span-2 btn font-Dana bg-red-500' >حذف پوستر</button>
                <button onClick={() => updatePoster(receivedData.id)} className=' col-span-6 sm:col-span-2 btn font-Dana' >ثبت تغیرات</button>
            </div>
        </div>
    )
}
